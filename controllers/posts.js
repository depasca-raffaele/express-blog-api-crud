...
function validatePostPayload(payload, { mode, currentId = null } = {}) {
    ...
    if (isCreate && 'id' in payload) {
        errors.push('Non devi passare id in creazione');
    }
    ...
    if (needsField('title') && !isNonEmptyString(payload.title)) {
        errors.push('title è obbligatorio e deve essere una stringa non vuota');
    }

    if (needsField('content') && !isNonEmptyString(payload.content)) {
    ...
    if (needsField('tags')) {
        ...
        } else if (payload.tags.some((tag) => !isNonEmptyString(tag))) {
    ...
    if (needsField('published') && typeof payload.published !== 'boolean') {
        errors.push('published deve essere boolean');
    }

    if (needsField('prep_time') && (!Number.isInteger(payload.prep_time) || payload.prep_time <= 0)) {
        errors.push('prep_time deve essere un intero positivo');
    }

    if (needsField('created_at') && !isValidIsoDateString(payload.created_at)) {
...
export function update(request, response) {
    ...
    const { errors, conflicts } = validatePostPayload(request.body, {
        mode: 'update',
        currentId: id,
    });
